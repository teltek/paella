            ...
            paella.player.videoContainer.paused()
                .then((paused) => {
                    ...
                    if (paused) {
                        ...
                        // No pause onclick (live)
                        //paella.player.pause();
                    }
                });

            var pluginVol = paella.pluginManager.getPlugin("es.upv.paella.volumeRangePlugin")
            if (pluginVol._preMutedVolume) {
                paella.player.videoContainer.setVolume(pluginVol._preMutedVolume)
            } else {
                paella.player.videoContainer.setVolume(1)
            }
        });
...
class StreamProvider {
    constructor(videoData) {
        ...
        this._autoplay = base.parameters.get('autoplay')!=='false';// || this.isLiveStreaming;
        this._startTime = 0;
    }
